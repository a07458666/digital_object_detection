...
def save_one_json(predn, jdict, path, class_map):
    # Save one JSON result {"image_id": 42, "category_id":
    # 18, "bbox": [258.15, 41.29, 348.26, 243.78], "score": 0.236}
...
def process_batch(detections, labels, iouv):
    """
    Return correct predictions matrix. Both sets
    of boxes are in (x1, y1, x2, y2) format.
    Arguments:
...
def run(
    ...
    if training:  # called by train.py
        ...
        if pt:
            ...
            LOGGER.info(
                f"Forcing --batch-size 1"
                f"square inference shape(1,3,{imgsz},{imgsz})"
            )
    ...
    if not training:
        ...
        LOGGER.info(
            ...
            f"%.1fms NMS per image at shape {shape}" % t
        )
    ...
    if not training:
        lentxt = len(list(save_dir.glob('labels/*.txt')))
        s = (
            f"\n{lentxt} labels saved to {save_dir / 'labels'}"
            if save_txt
            else ""
        )
...
def main(opt):
    ...
    if opt.task in ("train", "val", "test"):  # run normally
        ...
        if opt.conf_thres > 0.001:
            LOGGER.info(
                f"WARNING: confidence threshold {opt.conf_thres} >> 0.001"
                "will produce invalid mAP values."
            )
        run(**vars(opt))
