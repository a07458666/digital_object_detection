...
Usage:
    ...
    --name svhw_yolov5x_pre_320
    --weights yolov5x.pt --imgsz 320
"""
import argparse
import math
import os
...
def train(hyp, opt, device, callbacks):
    ...
    if pretrained:
        ...
        LOGGER.info(
            f"Transferred {len(csd)}/{len(model.state_dict())}"
            f"tems from {weights}"
        )  # report
    else:
    ...
    if pretrained:
        ...
        if resume:
            assert (
                start_epoch > 0
            ), f"{weights} training to {epochs}"
            f"epochs is finished, nothing to resume."
        if epochs < start_epoch:
            LOGGER.info(
                f"{weights} has been trained for {ckpt['epoch']}"
                f"epochs. Fine-tuning for {epochs} more epochs."
            )
    ...
    if cuda and RANK == -1 and torch.cuda.device_count() > 1:
        LOGGER.warning(
            "WARNING: DP not recommended,"
            "use torch.distributed.run\n"
            "for best DDP Multi-GPU results.\n"
    ...
    for epoch in range(
        ...
        for i, (
            ...
            if ni <= nw:
                xi = [0, nw]  # x interp
                # compute_loss.gr = np.interp(ni, xi, [0.0, 1.0])
                # iou loss ratio (obj_loss = 1.0 or iou)
                accumulate = max(
                    1, np.interp(ni, xi, [1, nbs / batch_size]).round()
                )
                for j, x in enumerate(optimizer.param_groups):
                    # bias lr falls from 0.1 to lr0, all other
                    # lrs rise from 0.0 to lr0
                    x["lr"] = np.interp(
                        ni,
                        xi,
                        [
            ...
            if RANK in [-1, 0]:
                ...
                mem = f"{memory / 1E9 if cuda_avai else 0:.3g}G"  # (GB)
...
def main(opt, callbacks=Callbacks()):
    ...
    # Evolve hyperparameters (optional)
        ...
        LOGGER.info(
            ...
            f"Use best hyperparameters example:"
            f"$ python train.py --hyp {evolve_yaml}"
        )
